...
def main():
    ...
    if task == "refgene":
        ...
        addAbsPath = [0, 1, 0, 3]
        message = "delongseq -task refgene -ref <reference_file> -format <reference_file_format(gtf/ucsc)> -out <output_file>"
    ...
    if task == "quantify":
        ...
        message = "delongseq -task quantify -refgene <refgene_file> -bam <bam_file> -out <output_file> -max_distance <max distance> -f_weight <weight of F function>"
        inputs = my.parse_argument(validArgList, addAbsPath, message)
        refFile = inputs[1]
        bamFile = inputs[2]
        outFile = inputs[3]
